...
export async function POST(request: NextRequest) {
  try {
    ...
    // The record is already saved at this point, so an email failure must not
    // turn the submission into an error for the user.
    try {
      ...
      console.error('Failed to send announcement notification email(s):', emailError);
    }
